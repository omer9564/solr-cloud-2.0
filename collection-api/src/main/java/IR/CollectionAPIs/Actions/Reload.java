...
/**
 * {@link SolrAction} this is a class that expose a Reload collection API
 *
 * @see SolrAction
 */
public class Reload implements SolrAction {
    private static final Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

    private String collectionName;
    private CloudSolrClient solrClient;

    public Reload(CloudSolrClient solrClient) {
        this.solrClient = solrClient;
    }

    public Reload setCollectionName(String collectionName) {
    ...
    public SolrResponse execute() throws IOException, SolrServerException, RuntimeException {
        var solrAdminCommand = CollectionAdminRequest.reloadCollection(collectionName);
        logger.info("created reload request");
