...
/**
 * {@link SolrAction} this is a class that expose a deletion of alias API
 *
 * @see SolrAction
 */
public class DeleteAlias implements SolrAction {
    private static final Logger logger = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

    private String aliasName;
    private CloudSolrClient solrClient;

    public DeleteAlias(CloudSolrClient solrClient) {
    ...
    public SolrResponse execute() throws IOException, SolrServerException, RuntimeException {
        var solrAdminCommand = CollectionAdminRequest.deleteAlias(aliasName);
        logger.info("created alias deletion request");
